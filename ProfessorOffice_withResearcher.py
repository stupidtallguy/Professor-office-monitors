...
class App(tk.Tk):
    def __init__(self):
        ...
        self.log = tk.Text(right, height=20, width=42, state="disabled")
    ...
    def _build_queues(self):
        ...
        ttk.Label(col0, text="Returning Researchers (highest)", font=("Helvetica", 12, "bold")).pack(anchor="w")
        self.rr_list = tk.Listbox(col0, height=10)
        self.rr_list.pack(fill="x", pady=(4, 12))

        # Column 1: New Researchers
        col1 = ttk.Frame(self, padding=12)
        col1.grid(row=0, column=1, sticky="nsew")
        ttk.Label(col1, text="Researchers (new arrivals)", font=("Helvetica", 12, "bold")).pack(anchor="w")
        self.rn_list = tk.Listbox(col1, height=10)
        self.rn_list.pack(fill="x", pady=(4, 12))
